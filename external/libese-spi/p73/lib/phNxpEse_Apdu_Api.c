 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEse_7816_Transceive(pphNxpEse_7816_cpdu_t pCmd, pphNxpEse_7816_rpdu_t pRsp)
{
    ...
    {
        NXPLOG_ESELIB_E(" %s Invalid prameter \n", __FUNCTION__);
        status = ESESTATUS_INVALID_PARAMETER;
    }
    else if(pCmd->cpdu_type > 1)
    {
        NXPLOG_ESELIB_E(" %s Invalid cpdu type \n", __FUNCTION__);
    ...
    else if(0 < pCmd->le_type && NULL == pRsp->pdata)
    {
        ...
        * provided by application */
        NXPLOG_ESELIB_E(" %s Invalid response buffer \n", __FUNCTION__);
    ...
    else
    {
        ...
        if (ESESTATUS_SUCCESS == status)
        {
            ...
            if (ESESTATUS_SUCCESS != status)
            {
                NXPLOG_ESELIB_E(" %s phNxpEse_Transceive Failed \n", __FUNCTION__);
            }
            else
            {
                if ((pRspTrans.len > 0) && (pRspTrans.p_data != NULL))
                {
                    ...
                    NXPLOG_ESELIB_D("pRsp->len %d", pRsp->len);
                    ...
                    else
                    {
                       /* if application response buffer is null and data is present */
                        NXPLOG_ESELIB_E("Invalid Res buffer");
                        status = ESESTATUS_FAILED;
                    }
                    NXPLOG_ESELIB_D("Freeing memroy pRspTrans.p_data ");
                ...
                {
                    NXPLOG_ESELIB_E("pRspTrans.len error = %d", pRspTrans.len);
                    status = ESESTATUS_FAILED;
                }
            }
            if (pCmd_data != NULL)
            {
                NXPLOG_ESELIB_D("Freeing memory pCmd_data");
 ...
 *
 * \param[in]       pphNxpEse_7816_cpdu_t pCmd- Structure pointer passed from application
        ...
        uint8_t **pcmd_data, uint32_t *cmd_len)
{
    ...
    NXPLOG_ESELIB_D("%s  pCmd->lc = %d, pCmd->le_type = %d",
    ...
    if (pCmd->lc > 0)
    {
        ...
        if (pCmd->pdata == NULL)
        {
            NXPLOG_ESELIB_E("%s Invalide data buffer from application ", __FUNCTION__);
    ...
    {
        lc_len = 0;
        NXPLOG_ESELIB_D("%s lc (data) field is not present %d", __FUNCTION__, pCmd->lc);
    }
    ...
    if (pCmd->le_type > 0)
    {
        ...
        else if ((pCmd->le_type == 2 || pCmd->le_type == 3))
        {
            ...
            {
                NXPLOG_ESELIB_E("%s wrong LE type  %d", __FUNCTION__, pCmd->le_type);
        ...
        {
            NXPLOG_ESELIB_E("%s wrong cpdu_type value %d", __FUNCTION__, pCmd->cpdu_type);
    ...
    {
        le_len = 0;
        NXPLOG_ESELIB_D("%s le field is not present", __FUNCTION__);
    }
    NXPLOG_ESELIB_D("%s cmd_total_len = %d, le_len = %d, lc_len = %d",
            __FUNCTION__, cmd_total_len, le_len, lc_len);

    pbuff = (uint8_t *)phNxpEse_calloc(cmd_total_len, sizeof(uint8_t));
    if (pbuff == NULL)
    {
        NXPLOG_ESELIB_D("%s Error allocating memory", __FUNCTION__);
    ...
    *(pbuff + index) = pCmd->p2;
    ...
    NXPLOG_ESELIB_D("Exit %s cmd_total_len = %d, index = %d", __FUNCTION__, index, cmd_total_len);
