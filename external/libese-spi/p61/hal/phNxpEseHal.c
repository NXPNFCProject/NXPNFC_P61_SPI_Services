 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEseP61_open( ese_stack_data_callback_t *p_data_cback, phNxpEseP61_initParams initParams)
{
    ...
    /* Setting init mode for controlling ESE powering (dummy APDU)*/
    if(ESE_MODE_OSU == initParams.initMode)
    {
        NXPLOG_SPIHAL_D("Going to phNxpEseP61_SPM_SetState: SPM_STATE_JCOP_DOWNLOAD");
        phNxpEseP61_SPM_SetState(SPM_STATE_JCOP_DOWNLOAD);
    }
    ...
    {/*A thread created using pthread needs to be either joined(phtread_join) or detached
     so that the resources are released after completion of task assigned to thread.*/
    int thread_ret;
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    thread_ret = pthread_create(&nxpesehal_ctrl.client_thread, &attr,
            phNxpEseP61_client_thread, &nxpesehal_ctrl);
    if(thread_ret != 0)
    {
        NXPLOG_SPIHAL_E("pthread_create ESE HAL failed");
        wConfigStatus = phTmlEse_Shutdown();
        nxpesehal_ctrl.p_ese_stack_data_cback = NULL;
        goto clean_and_return;
    }
    pthread_attr_destroy(&attr);
    NXPLOG_SPIHAL_E("ESE HAL Thread created");
    }
 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEseP61_openPrioSession(ese_stack_data_callback_t *p_data_cback,
    uint32_t timeOut, phNxpEseP61_initParams initParams)
{
    phOsalEse_Config_t tOsalConfig;
    phTmlEse_Config_t tTmlConfig;
    ESESTATUS wConfigStatus = ESESTATUS_SUCCESS;
