...
/******************************************************************************
 * Function         phNxpEseP61_SPM_SetState
 *
 * Description      This function set the power state of P61
 *
 * Returns          On Success SPMSTATUS_SUCCESS else proper error code
 *
 ******************************************************************************/
SPMSTATUS phNxpEseP61_SPM_SetState(spm_state_t current_spm_state)
{
    ...
    SPMSTATUS wSpmStatus = SPMSTATUS_SUCCESS;
    p61_access_state_t p61_current_state;
    switch(current_spm_state)
    {
        case SPM_STATE_JCOP_DOWNLOAD:
            p61_current_state = P61_STATE_JCOP_DWNLD;
            break;
        default:
            ALOGE(" %s : Invalid state", __FUNCTION__);
    }
    /* reset the p61 */
    ret = ioctl(fd_p61_device, P61_SET_SPM_STATUS, p61_current_state);
    if(ret < 0)
    {
        ALOGE("%s : failed errno = 0x%x", __FUNCTION__, errno);
        if(errno == -EBUSY)
...
static spm_state_t phNxpEseP61_StateMaping(p61_access_state_t p61_current_state)
{
    ...
    case P61_STATE_SPI_PRIO_END:
        ...
        break;
    case P61_STATE_JCOP_DWNLD:
        current_spm_state = SPM_STATE_JCOP_DOWNLOAD;
        ALOGD("%s : P61_STATE_SPI_PRIO_END", __FUNCTION__);
        break;
